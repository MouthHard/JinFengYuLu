import type { SceneryData } from '@/types/travelGuide';

export const beijingData: SceneryData = {
  overview: {
    totalSpots: 220,
    natureSpots: 90,
    cultureSpots: 130,
    topAttractions: [
      {
        name: '故宫',
        image: 'https://picsum.photos/id/1076/600/400',
        description: '中国明清两代的皇家宫�?是世界上现存规模最大、保存最为完整的木质结构古建筑之一',
      },
      {
        name: '长城',
        image: 'https://picsum.photos/id/1077/600/400',
        description: '中国古代伟大的防御工�?是中华民族的象征',
      },
      {
        name: '颐和�?,
        image: 'https://picsum.photos/id/1080/600/400',
        description: '中国现存规模最大、保存最完整的皇家园�?,
      },
    ],
  },
  spots: [
    {
      id: 'forbiddenCity',
      name: '故宫',
      description: '中国明清两代的皇家宫�?是世界上现存规模最大、保存最为完整的木质结构古建筑之一',
      image: 'https://picsum.photos/id/1076/600/400',
      location: '北京市东城区景山前街4�?,
      type: 'culture',
      bestTime: '春季',
      tags: ['历史', '文化', '建筑', '皇家'],
      rating: 4.9,
    },
    {
      id: 'greatWall',
      name: '长城',
      description: '中国古代伟大的防御工�?是中华民族的象征',
      image: 'https://picsum.photos/id/1077/600/400',
      location: '北京市延庆区八达岭长�?,
      type: 'nature',
      bestTime: '秋季',
      tags: ['历史', '自然', '建筑', '世界遗产'],
      rating: 4.8,
      landscapeType: 'mountain',
      landscapeCategory: ['地理景观'],
      landformTags: ['山地景观'],
    },
    {
      id: 'summerPalace',
      name: '颐和�?,
      description: '中国现存规模最大、保存最完整的皇家园�?,
      image: 'https://picsum.photos/id/1080/600/400',
      location: '北京市海淀区新建宫门路19�?,
      type: 'culture',
      bestTime: '春季',
      tags: ['园林', '历史', '皇家', '建筑'],
      rating: 4.7,
    },
  ],
  seasonal: [
    {
      name: '春季',
      description: '春暖花开,故宫、颐和园百花齐放,是游览皇家园林的最佳时�?,
      image: 'https://picsum.photos/id/1090/600/400',
      activities: ['赏花', '踏青', '故宫游览', '颐和园赏�?],
    },
    {
      name: '夏季',
      description: '绿树成荫,长城、香山避暑胜�?适合户外活动和登�?,
      image: 'https://picsum.photos/id/1091/600/400',
      activities: ['长城徒步', '香山避暑', '天坛纳凉', '北海公园游船'],
    },
    {
      name: '秋季',
      description: '秋高气爽,香山红叶满山,八达岭长城红叶漫�?是观赏红叶的最佳季�?,
      image: 'https://picsum.photos/id/1092/600/400',
      activities: ['香山赏红�?, '长城赏秋', '颐和园秋�?, '天坛秋游'],
    },
    {
      name: '冬季',
      description: '银装素裹,故宫雪景如画,什刹海冰场开�?体验北国风光',
      image: 'https://picsum.photos/id/1093/600/400',
      activities: ['故宫赏雪', '什刹海滑冰', '颐和园雪�?, '庙会体验'],
    },
  ],
  activities: [
    {
      id: '1',
      name: '故宫深度�?,
      description: '深入了解故宫的历史文化和建筑艺术',
      image: 'https://picsum.photos/id/1094/600/400',
      category: 'culture',
      location: '故宫博物�?,
      duration: '3-4小时',
      difficulty: '简�?,
      type: '文化体验',
    },
    {
      id: '2',
      name: '长城徒步',
      description: '徒步长城,感受古代防御工程的宏�?,
      image: 'https://picsum.photos/id/1095/600/400',
      category: 'nature',
      location: '八达岭长�?,
      duration: '4-6小时',
      difficulty: '中等',
      type: '户外活动',
    },
  ],
  routes: [
    {
      id: '1',
      name: '北京经典一日游',
      title: '北京经典一日游',
      description: '游览北京最著名的景�?,
      duration: '1�?,
      difficulty: '简�?,
      highlights: ['故宫', '天坛', '颐和�?],
      spots: ['故宫', '天坛', '颐和�?],
      tags: ['经典', '文化', '历史'],
      image: 'https://picsum.photos/id/1076/600/400',
      rating: 4.8,
      bestTime: '春季',
      traffic: '地铁/公交',
      suitableFor: '所有游�?,
    },
    {
      id: '2',
      name: '长城文化之旅',
      title: '长城文化之旅',
      description: '探索长城的历史和文化',
      duration: '2�?,
      difficulty: '中等',
      highlights: ['八达岭长�?, '慕田峪长�?],
      spots: ['八达岭长�?, '慕田峪长�?],
      tags: ['历史', '自然', '徒步'],
      image: 'https://picsum.photos/id/1077/600/400',
      rating: 4.7,
      bestTime: '秋季',
      traffic: '旅游大巴',
      suitableFor: '户外爱好�?,
    },
  ],
};

export const beijingCustomsData = {
  activities: [
    {
      id: '1',
      name: '京剧表演',
      description: '中国国粹京剧表演,体验传统艺术魅力',
      image: 'https://picsum.photos/id/1094/600/400',
      category: 'culture',
      location: '长安大戏�?,
      duration: '2-3小时',
      difficulty: '简�?,
      type: '文化体验',
    },
    {
      id: '2',
      name: '北京烤鸭制作体验',
      description: '学习北京烤鸭的传统制作工�?,
      image: 'https://picsum.photos/id/1080/600/400',
      category: 'culture',
      location: '全聚德烤鸭店',
      duration: '3-4小时',
      difficulty: '中等',
      type: '美食体验',
    },
  ],
};

export const beijingIntangibleHeritage = [
  {
    id: '1',
    name: '京剧',
    description: '中国国粹,集唱、念、做、打于一体的传统戏曲',
    level: 'national',
    category: '传统戏剧',
    protectionUnit: '北京京剧�?,
    features: ['传统戏剧', '国家级非�?, '国粹艺术'],
    icon: '🎭',
    image: 'https://picsum.photos/id/1094/600/400',
  },
  {
    id: '2',
    name: '北京烤鸭制作技�?,
    description: '北京特色美食制作技�?历史悠久',
    level: 'national',
    category: '传统技�?,
    protectionUnit: '全聚德烤鸭店',
    features: ['传统技�?, '国家级非�?, '美食文化'],
    icon: '🍗',
    image: 'https://picsum.photos/id/1080/600/400',
  },
];

export const beijingFolkCustomsElements = [
  {
    name: '老北京庙�?,
    description: '传统民俗活动,集祭祀、娱乐、购物于一�?,
    image: 'https://picsum.photos/id/1079/600/400',
    tags: ['传统民俗', '节庆活动', '非遗文化', '京味文化', '民俗体验'],
  },
  {
    name: '北京胡同',
    description: '北京传统民居形式,承载着丰富的历史文�?,
    image: 'https://picsum.photos/id/1078/600/400',
    tags: ['历史街区', '传统民居', '京味文化', '城市记忆', '文化遗产'],
  },
];

export const beijingFestivals = [
  {
    id: '1',
    name: '春节庙会',
    description: '农历新年期间举行的传统民俗活�?,
    month: '2�?,
    day: '1�?,
    location: '地坛公园',
    isUnique: true,
  },
  {
    id: '2',
    name: '元宵节灯�?,
    description: '农历正月十五举行的传统灯展活�?,
    month: '2�?,
    day: '15�?,
    location: '故宫',
    isUnique: true,
  },
  {
    id: '3',
    name: '中秋节赏�?,
    description: '传统节日,家人团聚赏月吃月�?,
    month: '9�?,
    day: '21�?,
    location: '颐和�?,
    isUnique: false,
  },
];

export default beijingData;
